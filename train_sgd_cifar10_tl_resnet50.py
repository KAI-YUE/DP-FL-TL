...
def federated_learning(config, logger):
    ...
    log_path = './cifar10_tl_dpsgd_trainacc_log'
    if not os.path.exists(log_path):
        os.makedirs(log_path)
    trainacc_file = open(log_path + '/cifar10_tlalpha1trainacc_file_fed_{}_{}_{}_{}_{}_{}_{:.4f}_{}_{}_{}.dat'.format(config.user_num, config.global_lr_init, config.unbiased_indicator, config.compressor, config.aggregator, config.clipping_bound, mu, config.A, config.B, config.mc), "w")
    ...
    log_path = './cifar10_tl_dpsgd_testacc_log'
    if not os.path.exists(log_path):
        os.makedirs(log_path)
    testacc_file = open(log_path + '/cifar10_tlalpha1testacc_file_fed_{}_{}_{}_{}_{}_{}_{:.4f}_{}_{}_{}.dat'.format(config.user_num, config.global_lr_init, config.unbiased_indicator, config.compressor, config.aggregator, config.clipping_bound, mu, config.A, config.B, config.mc), "w")
...
if __name__ == "__main__":
    config_file = "config/fedavgcifar10_tl_resent50.yaml"
    # mu_set = [0.1, 0.5]
    # for mu in mu_set:
    #     for m_c in range(5):
