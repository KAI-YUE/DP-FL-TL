...
class MyAttackerDPOptimizer(DPOptimizer):
    def __init__(self, config, dp_optimizer):
        self.add_noise = config.attacker_add_noise
...
def prepare_myattackerdp_optimizer(config, optimizer):
    attacker_dp_optimizer = MyAttackerDPOptimizer(config, optimizer)
    return attacker_dp_optimizer